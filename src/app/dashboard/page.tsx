import { DASHBOARD_STATS, RECENT_INQUIRIES } from "@/src/lib/constants";
import { ImageIcon, PlusCircle, Users, Zap } from "lucide-react";

export default function DashboardPage(){
    return(
        <div className="w-full min-h-screen flex flex-col gap-5">
            <div>
                <h2>Welcome back, Admin</h2>
                <p>Here is what's happening with Himalayan Elite today</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {DASHBOARD_STATS.map((stat, index) => {
                const Icon = stat.icon;
                return (
                <div 
                    key={index} 
                    className="p-6 border border-secondary-200/50 hover:border-secondary-200 rounded-2xl flex flex-col justify-between shadow-sm"
                >
                    {/* Top Row: Icon and Optional Badge */}
                    <div className="flex items-center justify-between">
                    <div className={`p-2.5 rounded-xl ${stat.iconBg}`}>
                        <Icon size={20} />
                    </div>
                    </div>

                    {/* Bottom Row: Title and Value */}
                    <div className="mt-6">
                    <p className="font-bold text-neutral-400 uppercase tracking-wider">
                        {stat.title}
                    </p>
                    <h3 className="font-extrabold text-neutral-900 mt-1.5 tracking-tight">
                        {stat.value}
                    </h3>
                    </div>
                </div>
                );
            })}
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">

                {/* Left Hand: Recent Inquiries Table Container */}
                <div className="lg:col-span-2 border border-secondary-200/50 hover:border-secondary-200 rounded-2xl bg-white shadow-sm overflow-hidden">

                    <div className="p-6 flex items-center justify-between border-b border-neutral-100">
                        <h4 className="text-lg font-bold text-neutral-900">Recent Inquiries</h4>
                        <button className="text-sm font-semibold text-primary-600 hover:text-primary-700 transition-colors hover:cursor-pointer">
                        View All
                        </button>
                    </div>

                    <div className="overflow-x-auto">
                        <table className="w-full border-collapse text-left">
                        <thead>
                            <tr className="bg-neutral-50 border-b border-neutral-100 text-[11px] font-bold tracking-wider text-neutral-500 uppercase">
                            <th className="py-3 px-6">Full Name</th>
                            <th className="py-3 px-6">Subject</th>
                            <th className="py-3 px-6 text-center">Status</th>
                            <th className="py-3 px-6">Date</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-neutral-100 text-sm">
                            {RECENT_INQUIRIES.map((row) => (
                            <tr key={row.id} className="hover:bg-neutral-50/50 transition-colors">
                                {/* User profile with initials */}
                                <td className="py-4 px-6 flex items-center gap-3">
                                    <div className="h-9 w-9 rounded-full bg-secondary-50 flex items-center justify-center font-bold text-xs shrink-0">
                                        {row.initials}
                                    </div>
                                    <span className="font-bold text-neutral-900 whitespace-nowrap">{row.name}</span>
                                </td>
                                {/* Subject line text */}
                                <td className="py-4 px-6 text-neutral-600 max-w-[220px] font-medium">
                                    {row.subject}
                                </td>
                                {/* Normalized badge states */}
                                <td className="py-4 px-6 text-center">
                                    <span className={`inline-block text-xs font-black tracking-wider uppercase px-2.5 py-1 rounded-md ${
                                        row.status === "PENDING" 
                                        ? "bg-indigo-50 text-indigo-600/90" 
                                        : "bg-emerald-50 text-emerald-600/90"
                                    }`}>
                                        {row.status}
                                    </span>
                                </td>
                                {/* Datetime string */}
                                <td className="py-4 px-6 text-neutral-500 font-medium whitespace-nowrap">
                                    {row.date}
                                </td>
                            </tr>
                            ))}
                        </tbody>
                        </table>
                    </div>
                </div>

                <div className="flex flex-col gap-6">

                    <div className="p-4 border border-secondary-200/50 hover:border-secondary-200 rounded-2xl bg-white shadow-sm flex flex-col gap-4">

                        <div className="p-2 flex items-center gap-2 ">
                            <Zap size={20} className="text-primary-600"/>
                            <h4 className="font-bold text-neutral-900">Quick Actions</h4>
                        </div>

                        <button className="px-2 py-4 flex gap-2 items-center border border-secondary-200/50 hover:border-secondary-200 rounded-xl hover:cursor-pointer">
                            <PlusCircle size={20} className="text-neutral-900"/>
                            <span className="text-sm font-semibold text-neutral-900">Create New Package</span>
                        </button>

                        <button className="px-2 py-4 flex gap-2 items-center border border-secondary-200/50 hover:border-secondary-200 rounded-xl hover:cursor-pointer">
                            <ImageIcon size={20} className="text-neutral-900"/>
                            <span className="text-sm font-semibold text-neutral-900">Add Gallery Image</span>
                        </button>

                        <button className="px-2 py-4 flex gap-2 items-center border border-secondary-200/50 hover:border-secondary-200 rounded-xl hover:cursor-pointer">
                            <Users size={20} className="text-neutral-900"/>
                            <span className="text-sm font-semibold text-neutral-900">Manage Company Info</span>
                        </button>

                    </div>

                </div>

            </div>

        </div>
    )
}